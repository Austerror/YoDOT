...
function creatureSayCallback(cid, type, msg)
	...
	if msgcontains(msg,'paw') and getPlayerStorageValue(cid,3500) == -1 and talkState ~= 1 then
		npcHandler:say('Oh, so you\'re interested in the druid paws then. Bring me 10 wolf paws and 10 bear paws and ask me again about the {paw} addon.', cid)
		talkState = 1
	elseif msgcontains(msg,'paw') and getPlayerStorageValue(cid,3500) == -1 and talkState == 1 then
		if getPlayerItemCount(cid,5897) >= 10 and getPlayerItemCount(cid,5896) >= 10 then
			doPlayerRemoveItem(cid,5896,10)
			doPlayerRemoveItem(cid,5897,10)
			setPlayerStorageValue(cid,3500,1)
			doPlayerAddOutfit(cid,144,1) -- m
			doPlayerAddOutfit(cid,148,1) -- f
			doSendMagicEffect(getCreaturePosition(cid), 12)
			npcHandler:say('There you are, the {paw} addon is now yours.', cid)
			talkState = 0
		else
			npcHandler:say('You don\'t seem to have all the items.', cid)
			talkState = 0
		end
	end
return true
